...
void PrintState(hashState *state)
{
	int i;
	for (i=nSBox-1; i>=0; i--)
		debug_printf("%02X", state->value[i]);
	debug_printf("\n");
}
...
bit16 retnuoCl(bit16 lfsr)
{
	switch(version)
	{
		...
		case 256512256:	
			...
			break;
		default :
			debug_printf("ErrorInverseCounter\n");
			break;			
	}	
...
HashReturn Squeeze(hashState *state)
{
	...
	for(int i = 0; i < R_SizeInBytes; i++) 
		debug_printf("%02X", state->messageblock[i]);
	debug_printf("\n\n");
#endif		
	Permute(state);
                        ...
                        bool unwrap)
{
    ...
    {
        debug_printf("Messages containing partial bytes not supported\n");
        return FAIL;
    }
